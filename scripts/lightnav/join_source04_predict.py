...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--run',type=Path,required=True);p.add_argument('--condition',choices=ORDER,required=True)
    a=p.parse_args();run=a.run.resolve();m=read(run/'conditions'/f'{a.condition}.json')
