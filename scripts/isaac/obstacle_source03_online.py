#!/usr/bin/env python3
"""Exactly two genuine OLD-at-POSE11 acquisitions; unchanged OSA02 pacing."""
import argparse
import os
...
from reconciliation.obstacle_source03 import reveal_due,first_post_reveal_allowed,REPETITIONS
...
class SuddenReveal(Visibility):
    ...
    def before_capture(self,ep,pose,state,active,last_activation):
        if not reveal_due(state['state_id'],state['sim_time_s'],active,last_activation,self.reveal is not None):return
        ...
        self.reveal=dict(start=before,completed=stamp(state['sim_time_s'],state['episode_time_s']),
            ...
            rule='first scheduled capture strictly after actual target OLD application',
...
def main():
    ...
    subprocess.run([str(ROOT/'.venv/bin/python'),str(ROOT/'scripts/run_obstacle_source_acquisition03.py'),'--run',str(run),'--mode','verify'],check=True,env=env)
