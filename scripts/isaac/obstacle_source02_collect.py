#!/usr/bin/env python3
"""OSA02: exactly one technical OFF episode; no Phase A requests."""
import argparse
...
def main():
    ...
    subprocess.run([str(ROOT/'.venv/bin/python'),str(ROOT/'scripts/run_obstacle_source_acquisition02.py'),'--run',str(parent),'--mode','verify'],check=True,env=env)
    assert read(parent/'generation_actual.json')['valid']
    if (run/'execution_start.json').exists():raise FileExistsError('one technical attempt, no retry')
