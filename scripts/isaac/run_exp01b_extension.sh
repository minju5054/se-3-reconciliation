...
CONFIG="${EXP01B_EXTENSION_CONFIG:-${REPOSITORY_ROOT}/configs/exp01b_extension.yaml}"
EXPERIMENT_ID="${EXP01B_EXTENSION_ID:-exp01b-extension-$(date -u +%Y%m%dT%H%M%SZ)}"
OUTPUT_DIR="${REPOSITORY_ROOT}/data/exp01b_extension/${EXPERIMENT_ID}"
RUNTIME_DIR="$(mktemp -d -t exp01b-extension-ipc-XXXXXX)"
...
cleanup() {
    ...
    if [[ -s "${SERVER_LOG}" ]]; then
        echo "[EXP-01B Extension] LightNav server log: ${SERVER_LOG}"
    else
        rm -f "${SERVER_LOG}"
    fi
}
...
echo "[EXP-01B Extension] Persistent LightNav server loaded and warmed."
...
wait "${server_pid}"
server_pid=""
"${RESEARCH_PYTHON}" "${REPOSITORY_ROOT}/scripts/summarize_exp01b_extension.py" \
    "${OUTPUT_DIR}"
echo "[EXP-01B Extension] Complete: ${OUTPUT_DIR}"
