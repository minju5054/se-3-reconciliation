...
# Scrub only the child process environment. Isaac Sim's python.sh subsequently establishes
# its own runtime library paths; those launcher-created paths are not removed.
exec env \
    -u LD_LIBRARY_PATH \
    -u PYTHONPATH \
    -u CUDA_HOME \
    ...
    "${REPOSITORY_ROOT}/scripts/isaac/jackal_trajectory_demo.py" \
    "$@"
